...
{ Btree: record numbers.  }
   DB_RECNUM = $0000010;
{ Recno: renumber on insert/delete.  }
   DB_RENUMBER = $0000020;
{ Btree: turn off reverse splits.  }
   DB_REVSPLITOFF = $0000040;
{ Recno: snapshot the input.  }
   DB_SNAPSHOT = $0000080;
{
   Flags private to the DB->stat methods.
  }
{ Clear stat after returning values.  }
   DB_STAT_CLEAR = $0000001;
{
   Flags private to DB->join.
  }
{ Don't try to optimize join.  }
   DB_JOIN_NOSORT = $0000001;
{
   Flags private to DB->verify.
  }
{ Salvage whatever could be data. }
   DB_AGGRESSIVE = $0000001;
{ Skip sort order/hashing check.  }
   DB_NOORDERCHK = $0000002;
